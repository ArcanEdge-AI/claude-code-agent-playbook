...
Copy the contents of this file into your global `CLAUDE.md` (`~/.claude/CLAUDE.md`, or `$CLAUDE_CONFIG_DIR/CLAUDE.md` if you've set that environment variable):
...
Open this prompt and run it in Claude Code:

```text
claude-prompts/setup-global-claude-support-system.md
```

That prompt creates a global support layer for reference docs, reusable skills, and custom subagents (`~/.claude/references/`, `~/.claude/skills/`, `~/.claude/agents/`) while avoiding duplicate global instructions.
...
```text
1. Paste the global custom instructions into your global CLAUDE.md.
